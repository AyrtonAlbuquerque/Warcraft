OnInit("Cyclone", function(requires)
    requires "Class"
    requires "Dummy"
    requires "Utilities"
    ...
    --                                          Lua API                                          --
    -- ----------------------------------------------------------------------------------------- --
    function CycloneUnit(source, target, duration, model, point, stack)
        CrowdControl.apply(CROWD_CONTROL_CYCLONE, source, target, 0, 0, duration, model, point, stack)
    end

    function IsUnitCycloned(target)
        return CrowdControl.applied(target, CROWD_CONTROL_CYCLONE)
    end

    -- ----------------------------------------------------------------------------------------- --
    --                                           System                                          --
    -- ----------------------------------------------------------------------------------------- --
    local Cyclone = Class(CrowdControl)

    local buff = S2A('BU09')
    local ability = S2A('U009')
    local order = "cyclone"
    ...
    function Cyclone:start(source, target, value, angle, duration, model, point, stack)
    ...
    function Cyclone:isApplied(unit)
        return GetUnitAbilityLevel(unit, buff) > 0
    end

    function Cyclone:getRemaining(unit)
        if timer[unit] then
    ...
    CROWD_CONTROL_CYCLONE = RegisterCrowdControl(Cyclone.allocate())
end)