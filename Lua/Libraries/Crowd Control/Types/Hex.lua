OnInit("Hex", function(requires)
    ...
    --                                          Lua API                                          --
    -- ----------------------------------------------------------------------------------------- --
    function HexUnit(source, target, duration, model, point, stack)
        CrowdControl.apply(CROWD_CONTROL_HEX, source, target, 0, 0, duration, model, point, stack)
    end

    function IsUnitHexed(target)
        return CrowdControl.applied(target, CROWD_CONTROL_HEX)
    end

    -- ----------------------------------------------------------------------------------------- --
    --                                           System                                          --
    -- ----------------------------------------------------------------------------------------- --
    local Hex = Class(CrowdControl)

    local buff = S2A('BU07')
    local ability = S2A('U007')
    local order = "hex"
    ...
    function Hex:start(source, target, value, angle, duration, model, point, stack)
    ...
    function Hex:isApplied(unit)
        return GetUnitAbilityLevel(unit, buff) > 0
    end

    function Hex:getRemaining(unit)
        if timer[unit] then
    ...
    CROWD_CONTROL_HEX = RegisterCrowdControl(Hex.allocate())
end)