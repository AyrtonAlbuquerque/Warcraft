OnInit("Purge", function(requires)
    requires "Class"
    requires "Dummy"
    requires "Utilities"
    ...
    --                                          Lua API                                          --
    -- ----------------------------------------------------------------------------------------- --
    function PurgeUnit(source, target, duration, model, point, stack)
        CrowdControl.apply(CROWD_CONTROL_PURGE, source, target, 0, 0, duration, model, point, stack)
    end

    function IsUnitPurged(target)
    ...
    --                                           System                                          --
    -- ----------------------------------------------------------------------------------------- --
    local Purge = Class(CrowdControl)

    local buff = S2A('BU06')
    local ability = S2A('U006')
    local order = "purge"
    ...
    function Purge:start(source, target, value, angle, duration, model, point, stack)
    ...
    function Purge:isApplied(unit)
        return GetUnitAbilityLevel(unit, buff) > 0
    end

    function Purge:getRemaining(unit)
        if timer[unit] then
    ...
    CROWD_CONTROL_PURGE = RegisterCrowdControl(Purge.allocate())
end)