OnInit("AttackSlow", function(requires)
    requires "Class"
    requires "Dummy"
    requires "Utilities"
    ...
    --                                          Lua API                                          --
    -- ----------------------------------------------------------------------------------------- --
    function SlowUnitAttack(source, target, amount, duration, model, point, stack)
        CrowdControl.apply(CROWD_CONTROL_SLOW_ATTACK, source, target, amount, 0, duration, model, point, stack)
    end

    function IsUnitAttackSlowed(target)
        return CrowdControl.applied(target, CROWD_CONTROL_SLOW_ATTACK)
    end

    -- ----------------------------------------------------------------------------------------- --
    --                                           System                                          --
    -- ----------------------------------------------------------------------------------------- --
    local AttackSlow = Class(CrowdControl)

    local buff = S2A('BU02')
    local ability = S2A('U002')
    local order = "cripple"
    ...
    function AttackSlow:start(source, target, value, angle, duration, model, point, stack)
        if duration > 0 and target then
            ...
            BlzSetAbilityRealLevelField(spell, ABILITY_RLF_ATTACK_SPEED_REDUCTION_PERCENT_CRI2, 0, value)
    ...
    function AttackSlow:finish(unit)
        if timer[unit] then
    ...
    function AttackSlow:isApplied(unit)
        return GetUnitAbilityLevel(unit, buff) > 0
    end

    function AttackSlow:getRemaining(unit)
        if timer[unit] then
    ...
    CROWD_CONTROL_SLOW_ATTACK = RegisterCrowdControl(AttackSlow.allocate())
end)