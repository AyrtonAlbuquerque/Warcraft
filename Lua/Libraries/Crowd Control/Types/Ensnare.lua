OnInit("Ensnare", function(requires)
    requires "Class"
    requires "Dummy"
    requires "Utilities"
    ...
    --                                          Lua API                                          --
    -- ----------------------------------------------------------------------------------------- --
    function EnsnareUnit(source,target, duration, model, point, stack)
        CrowdControl.apply(CROWD_CONTROL_ENSNARE, source, target, 0, 0, duration, model, point, stack)
    end

    function IsUnitEnsnared(target)
    ...
    --                                           System                                          --
    -- ----------------------------------------------------------------------------------------- --
    local Ensnare = Class(CrowdControl)

    local buff = S2A('BU05')
    local ability = S2A('U005')
    local order = "ensnare"
    ...
    function Ensnare:start(source, target, value, angle, duration, model, point, stack)
    ...
    function Ensnare:isApplied(unit)
        return GetUnitAbilityLevel(unit, buff) > 0
    end

    function Ensnare:getRemaining(unit)
        if timer[unit] then
    ...
    CROWD_CONTROL_ENSNARE = RegisterCrowdControl(Ensnare.allocate())
end)