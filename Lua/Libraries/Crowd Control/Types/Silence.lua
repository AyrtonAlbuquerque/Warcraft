OnInit("Silence", function(requires)
    requires "Class"
    requires "Dummy"
    requires "Utilities"
    ...
    --                                          Lua API                                          --
    -- ----------------------------------------------------------------------------------------- --
    function SilenceUnit(source, target, duration, model, point, stack)
        CrowdControl.apply(CROWD_CONTROL_SILENCE, source, target, 0, 0, duration, model, point, stack)
    end

    function IsUnitSilenced(target)
        return CrowdControl.applied(target, CROWD_CONTROL_SILENCE)
    end

    -- ----------------------------------------------------------------------------------------- --
    --                                           System                                          --
    -- ----------------------------------------------------------------------------------------- --
    local Silence = Class(CrowdControl)

    local buff = S2A('BU00')
    local ability = S2A('U000')
    ...
    function Silence:start(source, target, value, angle, duration, model, point, stack)
    ...
    function Silence:isApplied(unit)
        return GetUnitAbilityLevel(unit, buff) > 0
    end

    function Silence:getRemaining(unit)
        if timer[unit] then
    ...
    CROWD_CONTROL_SILENCE = RegisterCrowdControl(Silence.allocate())
end)