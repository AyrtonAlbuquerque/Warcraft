library Ensnare requires CrowdControl, Indexer, Dummy, Utilities
    globals
        integer CROWD_CONTROL_ENSNARE
    endglobals
    
    /* ----------------------------------------------------------------------------------------- */
    /*                                          JASS API                                         */
    /* ----------------------------------------------------------------------------------------- */
    function EnsnareUnit takes unit source, unit target, real duration, string model, string point, boolean stack returns nothing
        call CrowdControl.apply(CROWD_CONTROL_ENSNARE, source, target, 0, 0, duration, model, point, stack)
    endfunction

    function IsUnitEnsnared takes unit target returns boolean
        return CrowdControl.applied(target, CROWD_CONTROL_ENSNARE)
    endfunction

    /* ----------------------------------------------------------------------------------------- */
    /*                                           System                                          */
    /* ----------------------------------------------------------------------------------------- */
    private struct Ensnare extends CrowdControl
        private static constant integer buff = 'BU05'
        private static constant integer ability = 'U005'
        private static constant string order = "ensnare"
        ...
        private static method onInit takes nothing returns nothing
            set CROWD_CONTROL_ENSNARE = RegisterCrowdControl(thistype.allocate())
